...
def collect_rtstruct(df, ):
    ...
    temp = aggregate_images(df.fillna('-'), by=by)
    temp = temp[IMAGE_COLUMNS]

    contours = pd.merge(contours, temp, how='left', left_on='ReferenceSeriesInstanceUID', right_on='SeriesInstanceUID')\
